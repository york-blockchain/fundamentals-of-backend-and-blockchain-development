...
class InputForm {
    ...
    handleSubmit(event) {
        ...
        const secondNumberValue = this.secondNumber.value
        const operatorValue = this.operator.value

        if (!Validator.validate(firstNumberValue) ||
            !Validator.validate(secondNumberValue) ||
            !Validator.validate(operatorValue)) {
            alert("Invalid input")
            return
        }

        const result = Calculation.calculate(firstNumberValue, secondNumberValue, operatorValue)
        console.log(result)
    }
}
