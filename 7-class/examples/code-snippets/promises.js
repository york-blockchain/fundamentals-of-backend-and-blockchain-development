...
Promise.all([doThing2(), doThing3(), doThing4()])
...
ns.mxLookup = function (domain) {
  return ns.mxLookupLocal(domain).then(function (mxRecords) {
    if (!mxRecords) {
...
ns.checkSingleEmail = function (user, email, tag, ip) {
  return new Promise(function (fulfill, reject) {
    ...
    return ns
      .canUserCheckEmail(user, ip)
      .then(
        function () {
          // Local db lookup for cached result
          return ns.localEmailLookup(user, email, tag, ip);
        },
        function (err) {
          // Error (no credits or restricted by IP)
          reject(err);
        }
      )
      .then(function (json) {
        if (json) {
          // Return cached JSON response immediately
          return fulfill(json);
        }
